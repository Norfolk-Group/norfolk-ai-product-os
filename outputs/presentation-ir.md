...
Assets are registry entries, never anonymous URLs or copied binaries. Each entry has a stable ID, kind, Norfolk registry reference, SHA-256 digest, ownership and rights-review record, and accessibility treatment. Canonical assets are Norfolk-owned synthetic placeholders. Semantic-to-render compatibility is bidirectional: semantic `asset` content must use an `image` primitive carrying the same governed `assetId` and accepting only `image` or `svg`; semantic `chart` content must use a `chart` primitive carrying the same governed data asset and accepting only `data`. Conversely, `image` and `chart` primitives require those matching semantic content types. Other primitives cannot attach or stand in for an asset. JSON Schema enforces the local shape while the semantic validator enforces this cross-document relationship. A renderer must resolve assets before rendering, verify their hashes and governance, and fail closed on missing, changed, incompatible, unapproved, inaccessible, or remotely fetched assets.
